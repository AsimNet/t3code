...
function WorkingTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "working" }> }) {
  return (
    <div className="py-0.5 ps-1.5">
      <div className="flex items-center gap-2 pt-1 text-2xs text-muted-foreground/70 tabular-nums">
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  return (
    <section className="-mx-1 space-y-0.5 px-1 py-0.5" aria-label={groupLabel}>
      {!onlyToolEntries && (
        <p className="px-0.5 pb-0.5 font-medium text-2xs text-muted-foreground/65">{groupLabel}</p>
      )}
  ...
  return (
    <div className="mb-2 flex max-w-full items-center overflow-hidden rounded-lg border border-border/70 bg-background/70">
      ...
      <div className="min-w-0 px-2.5 py-2">
        ...
        <div
          className={cn(
            "flex items-center gap-2 text-3xs text-muted-foreground",
    ...
    >
      ...
      {expanded && canExpand && expandedBody ? (
        ...
        >
          ...
          <pre className="force-ltr max-h-64 cursor-text overflow-auto whitespace-pre-wrap break-words font-mono text-2xs leading-relaxed text-muted-foreground select-text">
            {expandedBody}
          </pre>
        </div>
      ) : null}
