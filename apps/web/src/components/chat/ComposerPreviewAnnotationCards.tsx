...
function TargetStat(props: { icon: ReactNode; count: number; label: string }) {
  return (
    <span
      className="inline-flex items-center gap-1 text-3xs font-medium text-muted-foreground"
        ...
        return (
          ...
          >
            {image?.previewUrl ? (
              ...
              <span className="grid size-10 shrink-0 place-items-center border-e border-border/70 text-blue-500">
                ...
                {elementLabels.length > 0 ? (
                  <div className="flex min-w-0 items-center gap-1">
                    {elementLabels.slice(0, 2).map(({ id, label }) => (
                      <span
                        key={id}
                        className="max-w-40 truncate font-mono text-3xs text-foreground/65"
                      >
                        {label}
                      </span>
                    ))}
            ...
            <button
              ...
              className="absolute end-1.5 top-1.5 grid size-5 place-items-center rounded text-muted-foreground/60 transition hover:bg-muted hover:text-foreground"
