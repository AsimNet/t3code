  ...
  const selectedScopeLabel =
    selectedTurnId === null
      ? selectedGitScope === "unstaged"
        ? t("panel.diff.scope.workingTree")
        : t("panel.diff.scope.branchChanges")
      : selectedTurn?.turnId === latestTurn?.turnId
        ? t("panel.diff.scope.latestTurn")
        : t("panel.diff.scope.turn", { count: selectedCheckpointTurnCount ?? "?" });
  ...
  const headerRow = (
    <>
      <div className="flex min-w-0 flex-1 items-center gap-3 [-webkit-app-region:no-drag]">
        <DropdownMenu>
          <DropdownMenuTrigger
            ...
            aria-label={t("panel.diff.scopeAria", { scope: selectedScopeLabel })}
          >
          ...
          <DropdownMenuContent align="start" className="w-60">
            ...
            >
              <span>{t("panel.diff.scope.workingTree")}</span>
            </DropdownMenuItem>
            <DropdownMenuItem
              className={
            ...
            >
              <span>{t("panel.diff.scope.branchChanges")}</span>
            </DropdownMenuItem>
            <DropdownMenuItem
              className={
            ...
            >
              <span>{t("panel.diff.scope.latestTurn")}</span>
            </DropdownMenuItem>
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>{t("panel.diff.scope.turnSubmenu")}</DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="w-64">
                {orderedTurnDiffSummaries.map((summary) => {
                  ...
                  return (
                    ...
                    >
                      <span>{t("panel.diff.scope.turn", { count: turnCount })}</span>
        ...
        {selectedTurnId === null && selectedGitScope === "branch" && selectedGitSource?.baseRef && (
          <div
            ...
            aria-label={t("panel.diff.comparingAria", {
          ...
          >
            ...
            >
              <ComboboxTrigger
                ...
                aria-label={t("panel.diff.baseRef.changeAria", {
                  base: selectedGitSource.baseRef,
                })}
              >
              ...
              >
                <div className="min-w-0 shrink-0 px-3 pt-2.5">
                  <div className="relative -translate-y-px border-b border-border/70 pb-1.5 transition-colors focus-within:border-ring">
                    <SearchIcon
                      aria-hidden="true"
                      className="pointer-events-none absolute top-1.5 start-0 size-4 shrink-0 text-muted-foreground/55"
                    />
                    <ComboboxInput
                      className="[&_input]:h-6.5 [&_input]:ps-5 [&_input]:font-sans [&_input]:leading-6.5"
                      inputClassName="force-ltr rounded-none bg-transparent text-sm"
                      placeholder={t("panel.diff.baseRef.searchPlaceholder")}
                      showTrigger={false}
                      size="sm"
                      unstyled
                      value={baseRefQuery}
                ...
                <div className="grid shrink-0 grid-cols-[1rem_minmax(0,1fr)] items-center gap-2 border-b border-border/70 ps-3 pe-6.5 pt-2 pb-1.5 font-medium text-3xs text-muted-foreground uppercase tracking-wide">
                  ...
                  <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_2rem] items-center">
                    <span>{t("panel.diff.baseRef.branchColumn")}</span>
                    <span className="text-end">{t("panel.diff.baseRef.remoteColumn")}</span>
                  </div>
                </div>
                <ComboboxEmpty>{t("panel.diff.baseRef.empty")}</ComboboxEmpty>
                <ComboboxList className="max-h-64 min-w-0 overflow-x-hidden">
                  ...
                  >
                    <span className="block min-w-0 truncate">
                      {t("panel.diff.baseRef.automatic")}
                    </span>
                  </ComboboxItem>
                      ...
                      >
                        <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_2rem] items-center overflow-hidden">
                          ...
                          {hasBoth ? (
                            ...
                            >
                              <Switch
                                aria-label={t("panel.diff.baseRef.useRemoteAria", {
                                  ref: choice.label,
                                })}
                                checked={useRemote}
                            ...
                            <span
                              ...
                              title={t("panel.diff.baseRef.remoteOnly")}
                            >
      ...
      <div className="flex shrink-0 items-center gap-1 [-webkit-app-region:no-drag]">
                ...
                <Button
                  ...
                  aria-label={
                    allDiffFilesCollapsed
                      ? t("panel.diff.expandAllFiles")
                      : t("panel.diff.collapseAllFiles")
                  }
        ...
        >
          <Toggle aria-label={t("panel.diff.view.stacked")} value="stacked">
            <Rows3Icon className="size-3" />
          </Toggle>
          <Toggle aria-label={t("panel.diff.view.split")} value="split">
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <Toggle
                aria-label={
                  wordWrap ? t("panel.diff.wrap.disableAria") : t("panel.diff.wrap.enableAria")
                }
                variant="outline"
                size="xs"
                pressed={wordWrap}
          ...
          <TooltipPopup side="top">
            {wordWrap ? t("panel.diff.wrap.disable") : t("panel.diff.wrap.enable")}
          </TooltipPopup>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger
              ...
              <Toggle
                aria-label={
                  diffIgnoreWhitespace
                    ? t("panel.diff.whitespace.show")
                    : t("panel.diff.whitespace.hide")
                }
                variant="outline"
                size="xs"
    ...
    <DiffPanelShell mode={mode} header={headerRow}>
      {!activeThread ? (
        <div className="flex flex-1 items-center justify-center px-5 text-center text-xs text-muted-foreground/70">
          {t("panel.diff.empty.selectThread")}
        </div>
      ) : !isGitRepo ? (
        <div className="flex flex-1 items-center justify-center px-5 text-center text-xs text-muted-foreground/70">
          {t("panel.diff.empty.notGitRepo")}
        </div>
      ) : selectedTurnId !== null && orderedTurnDiffSummaries.length === 0 ? (
        <div className="flex flex-1 items-center justify-center px-5 text-center text-xs text-muted-foreground/70">
          {t("panel.diff.empty.noTurns")}
        </div>
      ) : (
        <>
          <div className="diff-panel-viewport flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
            {isSelectedPatchTruncated && (
              <p className="shrink-0 border-b border-border/70 bg-muted/40 px-3 py-1.5 text-2xs text-muted-foreground">
                {t("panel.diff.truncated")}
              </p>
            )}
            {selectedPatchError && !renderablePatch && (
              <div className="px-3">
                <p className="mb-2 text-2xs text-red-500/80">{selectedPatchError}</p>
              </div>
            )}
            {!renderablePatch ? (
              isLoadingSelectedPatch ? (
                <DiffPanelLoadingState
                  label={
                    selectedTurn
                      ? t("panel.diff.loading.checkpoint")
                      : selectedGitScope === "unstaged"
                        ? t("panel.diff.loading.workingTree")
                        : t("panel.diff.loading.branch")
                  }
                />
              ) : (
                <div className="flex h-full items-center justify-center px-3 py-2 text-xs text-muted-foreground/70">
                  <p>{hasNoNetChanges ? t("panel.diff.noNetChanges") : t("panel.diff.noPatch")}</p>
                </div>
              )
                  ...
                  renderHeaderPrefix={(fileDiff, fileKey, collapsed) => {
                    ...
                    return (
                      <Tooltip>
                        <TooltipTrigger
                          render={
                            <button
                              ...
                              aria-label={
                                collapsed
                                  ? t("panel.diff.file.expandAria", { path: filePath })
                                  : t("panel.diff.file.collapseAria", { path: filePath })
                              }
                        ...
                        <TooltipPopup side="top">
                          {collapsed ? t("panel.diff.file.expand") : t("panel.diff.file.collapse")}
                        </TooltipPopup>
                      </Tooltip>
                    );
                  }}
              ...
              <div className="min-h-0 flex-1 overflow-auto p-2">
                <div className="space-y-2">
                  <p className="text-2xs text-muted-foreground/75">{renderablePatch.reason}</p>
                  <pre
                    className={cn(
                      // A raw patch is code: keep it LTR even when the UI is not.
                      "force-ltr max-h-[72vh] rounded-md border border-border/70 bg-background/70 p-3 font-mono text-2xs leading-relaxed text-muted-foreground/90",
                      wordWrap
