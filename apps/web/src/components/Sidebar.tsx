...
const SIDEBAR_SORT_LABEL_KEYS: Record<SidebarProjectSortOrder, TranslationKey> = {
  updated_at: "sidebar.sort.lastUserMessage",
  created_at: "sidebar.sort.createdAt",
  manual: "sidebar.sort.manual",
};
const SIDEBAR_THREAD_SORT_LABEL_KEYS: Record<SidebarThreadSortOrder, TranslationKey> = {
  updated_at: "sidebar.sort.lastUserMessage",
  created_at: "sidebar.sort.createdAt",
};
...
function projectGroupingModeDescriptionKey(mode: SidebarProjectGroupingMode): TranslationKey {
  switch (mode) {
    case "repository":
      return "sidebar.grouping.repository.description";
    case "repository_path":
      return "sidebar.grouping.repositoryPath.description";
    case "separate":
      return "sidebar.grouping.separate.description";
  }
}
...
export const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const threadEnvironmentLabel = isRemoteThread
    ? (remoteEnvLabel ??
      (isDesktopLocalThread ? t("sidebar.environment.local") : t("sidebar.environment.remote")))
    : null;
    ...
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("sidebar.toast.previewOpenFailed"),
            description:
              error instanceof Error
                ? error.message
                : t("sidebar.toast.previewOpenFailedDescription"),
          }),
        );
      })();
    },
        ...
        void (async () => {
          ...
          if (result._tag === "Failure") {
            ...
            toastManager.add(
              stackedThreadToast({
                type: "error",
                title: t("sidebar.toast.threadActionFailed"),
            ...
            <Tooltip>
              <TooltipTrigger
                render={
                  <button
                    type="button"
                    aria-label={t("sidebar.openLocalhost", {
              ...
              <TooltipPopup side="top">
                {t("sidebar.openLocalhost", { port: discoveredPorts[0]?.port ?? "" })}
          ...
          >
            {isConfirmingArchive ? (
              <button
                ...
                aria-label={t("sidebar.confirmArchiveThread", { title: thread.title })}
                className="absolute top-1/2 end-1 inline-flex h-5 -translate-y-1/2 cursor-pointer items-center rounded-md bg-destructive/12 px-2 text-3xs font-medium text-destructive transition-colors hover:bg-destructive/18 focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-destructive/40"
              ...
              appSettingsConfirmThreadArchive ? (
                <div className="pointer-events-none absolute top-1/2 end-0.5 -translate-y-1/2 opacity-0 transition-opacity duration-150 max-sm:pointer-events-auto max-sm:opacity-100 group-hover/menu-sub-item:pointer-events-auto group-hover/menu-sub-item:opacity-100 group-focus-within/menu-sub-item:pointer-events-auto group-focus-within/menu-sub-item:opacity-100">
                  <button
                    ...
                    aria-label={t("sidebar.archiveThread", { title: thread.title })}
                ...
                {isRemoteThread && !isDesktopLocalThread && (
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <span
                          aria-label={threadEnvironmentLabel ?? t("sidebar.environment.remote")}
    ...
    >
      {shouldShowThreadPanel && showEmptyThreadState ? (
        <SidebarMenuSubItem className="w-full" data-thread-selection-safe>
          <div
            data-thread-selection-safe
            className="flex h-8 w-full translate-x-0 items-center px-2 text-start text-xs text-sidebar-muted-foreground/75"
          >
      ...
      {projectExpanded && hasOverflowingThreads && !isThreadListExpanded && (
        <SidebarMenuSubItem className="w-full">
          <SidebarMenuSubButton
            ...
            data-thread-selection-safe
            size="sm"
            className="h-8 w-full translate-x-0 justify-start px-2 text-start text-xs text-sidebar-muted-foreground/75 hover:bg-sidebar-row-hover hover:text-sidebar-foreground"
            onClick={() => {
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const { copyToClipboard: copyThreadIdToClipboard } = useCopyToClipboard<{
    ...
    onCopy: (ctx) => {
      toastManager.add({
        type: "success",
        title: t("sidebar.toast.threadIdCopied"),
    ...
    onError: (error) => {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("sidebar.toast.threadIdCopyFailed"),
          description: error instanceof Error ? error.message : t("sidebar.error.generic"),
        }),
      );
    },
  ...
  const { copyToClipboard: copyPathToClipboard } = useCopyToClipboard<{
    ...
    onCopy: (ctx) => {
      toastManager.add({
        type: "success",
        title: t("sidebar.toast.pathCopied"),
    ...
    onError: (error) => {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("sidebar.toast.pathCopyFailed"),
          description: error instanceof Error ? error.message : t("sidebar.error.generic"),
        }),
      );
    },
    ...
    async (member: SidebarProjectGroupMember) => {
      ...
      if (memberThreadCount > 0) {
        const warningToastId = toastManager.add(
          stackedThreadToast({
            type: "warning",
            title: t("sidebar.toast.projectNotEmpty"),
            description: t("sidebar.toast.projectNotEmptyDescription"),
            actionVariant: "destructive",
            actionProps: {
              children: t("sidebar.toast.deleteAnyway"),
              onClick: () => {
                void (async () => {
                  ...
                  const confirmed = await api.dialogs.confirm(
                    latestProjectThreads.length > 0
                      ? [
                          ...
                          t("sidebar.confirm.path", { path: member.workspaceRoot }),
                          ...(member.environmentLabel
                            ? [
                                t("sidebar.confirm.environment", {
                                  environment: member.environmentLabel,
                                }),
                              ]
                            : []),
                          t("sidebar.confirm.clearHistory.those"),
                          t("sidebar.confirm.onlyProjectEntry"),
                          t("sidebar.confirm.cannotUndo"),
                        ].join("\n")
                      : [
                          t("sidebar.confirm.removeProject", { title: member.title }),
                          t("sidebar.confirm.path", { path: member.workspaceRoot }),
                          ...(member.environmentLabel
                            ? [
                                t("sidebar.confirm.environment", {
                  ...
                  if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
                    ...
                    toastManager.add(
                      stackedThreadToast({
                        type: "error",
                        title: t("sidebar.toast.removeProjectFailed", { title: member.title }),
                        description:
                          error instanceof Error
                            ? error.message
                            : t("sidebar.toast.removeProjectUnknownError"),
                      }),
                    );
                  }
                })().catch((error) => {
      ...
      const message = [
        ...
        t("sidebar.confirm.path", { path: member.workspaceRoot }),
        ...(member.environmentLabel
          ? [t("sidebar.confirm.environment", { environment: member.environmentLabel })]
          : []),
        t("sidebar.confirm.onlyProjectEntry"),
      ].join("\n");
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        const error = squashAtomCommandFailure(result);
        const message =
          error instanceof Error ? error.message : t("sidebar.toast.removeProjectUnknownError");
        console.error("Failed to remove project", {
          projectId: member.id,
          environmentId: member.environmentId,
          ...safeErrorLogAttributes(error),
    ...
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        const clicked = await api.contextMenu.show(
          [
            buildTargetedItem("rename", t("sidebar.menu.rename")),
            buildTargetedItem("grouping", t("sidebar.menu.groupInto")),
            buildTargetedItem("copy-path", t("sidebar.menu.copyPath")),
            buildTargetedItem("delete", t("sidebar.menu.remove"), {
              destructive: true,
            }),
          ],
          {
      ...
      if (clicked === "archive") {
        ...
        for (const failure of archiveOutcome.followupFailures) {
          ...
          toastManager.add(
            stackedThreadToast({
              type: "error",
              title: t("sidebar.toast.threadArchivedNavigationFailed"),
              description: error instanceof Error ? error.message : t("sidebar.error.generic"),
            }),
          );
        }
        if (archiveOutcome.mutationFailure) {
          ...
          if (!isAtomCommandInterrupted(archiveOutcome.mutationFailure)) {
            ...
            toastManager.add(
              stackedThreadToast({
                type: "error",
                title: t("sidebar.toast.archiveThreadsFailed"),
                description: error instanceof Error ? error.message : t("sidebar.error.generic"),
              }),
            );
          }
      ...
      if (appSettingsConfirmThreadDelete) {
        const confirmed = await api.dialogs.confirm(
          [
            ...
            t("sidebar.confirm.clearHistory.these"),
          ].join("\n"),
        );
        if (!confirmed) return;
      }
        ...
        if (result._tag === "Failure") {
          if (!isAtomCommandInterrupted(result)) {
            ...
            toastManager.add(
              stackedThreadToast({
                type: "error",
                title: t("sidebar.toast.deleteThreadsFailed"),
                description: error instanceof Error ? error.message : t("sidebar.error.generic"),
              }),
            );
          }
      ...
      void (async () => {
        ...
        if (result._tag === "Failure") {
          ...
          toastManager.add(
            stackedThreadToast({
              type: "error",
              title: t("sidebar.toast.createThreadFailed"),
              description: error instanceof Error ? error.message : t("sidebar.error.generic"),
            }),
          );
        }
      ...
      void (async () => {
        ...
        if (clickedResult._tag === "Failure") {
          ...
          toastManager.add(
            stackedThreadToast({
              type: "error",
              title: t("sidebar.toast.chooseEnvironmentFailed"),
              description: error instanceof Error ? error.message : t("sidebar.error.generic"),
            }),
          );
          return;
    ...
    async (threadRef: ScopedThreadRef) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("sidebar.toast.archiveThreadFailed"),
            description: error instanceof Error ? error.message : t("sidebar.error.generic"),
          }),
        );
      }
  ...
  const commitRename = useCallback(
    async (threadRef: ScopedThreadRef, newTitle: string, originalTitle: string) => {
      ...
      if (trimmed.length === 0) {
        toastManager.add({
          type: "warning",
          title: t("sidebar.toast.threadTitleEmpty"),
        });
        finishRename();
        return;
      }
  ...
  const submitProjectRename = useCallback(async () => {
    ...
    if (trimmed.length === 0) {
      toastManager.add({
        type: "warning",
        title: t("sidebar.toast.projectTitleEmpty"),
      });
      return;
    }
    ...
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const clicked = await api.contextMenu.show(
        [
          ...
          { id: "copy-thread-id", label: t("sidebar.menu.copyThreadId") },
          { id: "delete", label: t("sidebar.menu.delete"), destructive: true, icon: "trash" },
        ],
        position,
      );
      ...
      if (clicked === "copy-path") {
        if (!threadWorkspacePath) {
          toastManager.add(
            stackedThreadToast({
              type: "error",
              title: t("sidebar.toast.pathUnavailable"),
              description: t("sidebar.toast.pathUnavailableDescription"),
            }),
          );
          return;
        }
    ...
    <>
      <div className="group/project-header relative">
        <SidebarMenuButton
          ...
          className={`pe-8 group-hover/project-header:bg-sidebar-row-hover group-hover/project-header:text-sidebar-foreground max-sm:pe-14 ${
          ...
          {!projectExpanded && projectStatus ? (
            <Tooltip>
              ...
              >
                ...
                <ChevronRightIcon className="absolute inset-0 m-auto size-3.5 text-muted-foreground/70 opacity-0 transition-opacity duration-150 group-hover/project-header:opacity-100 rtl:rotate-180" />
              </TooltipTrigger>
              <TooltipPopup side="top">{projectStatus.label}</TooltipPopup>
            </Tooltip>
          ) : (
            <ChevronRightIcon
              className={`-ms-0.5 size-3.5 shrink-0 text-muted-foreground/70 transition-transform duration-150 ${
                projectExpanded ? "rotate-90" : "rtl:rotate-180"
              }`}
            />
          )}
          ...
          <span className="flex min-w-0 flex-1 items-center gap-2">
            ...
            {project.groupedProjectCount > 1 ? (
              <span className="shrink-0 text-3xs text-muted-foreground/60">
                {t("sidebar.groupedProjectCount", { count: project.groupedProjectCount })}
              </span>
            ) : null}
          </span>
        </SidebarMenuButton>
        ...
        {project.environmentPresence === "remote-only" && (
          <Tooltip>
                ...
                <span
                  aria-label={
                    project.allRemoteMembersAreDesktopLocal
                      ? t("sidebar.project.localSandbox")
                      : t("sidebar.project.remote")
                  }
                  className="pointer-events-none absolute top-1 end-1.5 inline-flex size-5 items-center justify-center rounded-md text-muted-foreground/60 transition-opacity duration-150 max-sm:end-7 group-hover/project-header:opacity-0 group-focus-within/project-header:opacity-0 max-sm:group-hover/project-header:opacity-100 max-sm:group-focus-within/project-header:opacity-100"
                />
              }
            >
              {project.allRemoteMembersAreDesktopLocal ? (
            ...
            <TooltipPopup side="top">
              {project.allRemoteMembersAreDesktopLocal
                ? t("sidebar.project.localSandboxLabels", {
                    labels: project.remoteEnvironmentLabels.join(", "),
                  })
                : t("sidebar.project.remoteLabels", {
                    labels: project.remoteEnvironmentLabels.join(", "),
                  })}
            </TooltipPopup>
          </Tooltip>
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <div className="pointer-events-none absolute top-[calc(50%+1px)] end-0.5 -translate-y-1/2 opacity-0 transition-opacity duration-150 max-sm:pointer-events-auto max-sm:opacity-100 group-hover/project-header:pointer-events-auto group-hover/project-header:opacity-100 group-focus-within/project-header:pointer-events-auto group-focus-within/project-header:opacity-100">
                <button
                  type="button"
                  aria-label={t("sidebar.newThread.inProject", { project: project.displayName })}
          ...
          <TooltipPopup side="top">
            {newThreadShortcutLabel
              ? t("sidebar.newThread.withShortcut", { shortcut: newThreadShortcutLabel })
              : t("sidebar.newThread")}
          </TooltipPopup>
        </Tooltip>
      </div>
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {projectRenameTarget
                ? t("sidebar.renameProject.describeTarget", {
                    path: projectRenameTarget.workspaceRoot,
                  })
                : t("sidebar.renameProject.describeGeneric")}
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t("sidebar.grouping.title")}</DialogTitle>
            <DialogDescription>
              {projectGroupingTarget
                ? t("sidebar.grouping.describeTarget", {
                    path: projectGroupingTarget.workspaceRoot,
                  })
                : t("sidebar.grouping.describeGeneric")}
            </DialogDescription>
          </DialogHeader>
          <DialogPanel className="space-y-4">
            <div className="grid gap-1.5">
              ...
              >
                <SelectTrigger className="w-full" aria-label={t("sidebar.grouping.ruleAriaLabel")}>
                  <SelectValue>
                    {projectGroupingSelection === "inherit"
                      ? t("sidebar.grouping.useGlobalDefaultWith", {
                          mode: t(
      ...
      {connecting.length > 0 ? (
        ...
        >
          ...
          <AlertTitle className="text-xs font-medium text-foreground">
            {t("sidebar.local.connecting", { labels: connecting.join(", ") })}
          </AlertTitle>
        </Alert>
      ) : null}
      {failed.length > 0 ? (
        <Alert variant="warning" className="rounded-2xl border-warning/40 bg-warning/8">
          ...
          <AlertDescription>
            {failed
              ...
              .filter(Boolean)
              .join("; ") || t("sidebar.local.noResponse")}
          </AlertDescription>
        </Alert>
      ) : null}
    </SidebarGroup>
    ...
    <Menu>
      ...
      <MenuPopup align="end" side="bottom" className="min-w-52">
        <MenuGroup>
          ...
          >
            {(
              Object.entries(SIDEBAR_SORT_LABEL_KEYS) as Array<
                [SidebarProjectSortOrder, TranslationKey]
              >
            ).map(([value, labelKey]) => (
          ...
          >
            {(
              Object.entries(SIDEBAR_THREAD_SORT_LABEL_KEYS) as Array<
                [SidebarThreadSortOrder, TranslationKey]
              >
            ).map(([value, labelKey]) => (
              <MenuRadioItem key={value} value={value} className="min-h-7 py-1 sm:text-xs">
                {t(labelKey)}
          ...
          <div className="px-2 py-1">
            ...
            >
              <NumberFieldGroup className="h-7 rounded-md sm:h-6.5">
                <NumberFieldDecrement
                  aria-label={t("sidebar.visibleThreadCount.decrease")}
                ...
                />
                <NumberFieldIncrement
                  aria-label={t("sidebar.visibleThreadCount.increase")}
                  className="px-2 sm:px-2 [&_svg]:size-3.5"
                />
              </NumberFieldGroup>
            </NumberField>
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              ...
              >
                ...
                {commandPaletteShortcutLabel ? (
                  <Kbd className="h-4 min-w-0 rounded-sm px-1.5 text-3xs">
                    {commandPaletteShortcutLabel}
                  </Kbd>
                ) : null}
              </CommandDialogTrigger>
              ...
              <AlertAction>
                ...
                >
                  {desktopUpdateButtonAction === "download"
                    ? t("sidebar.update.downloadArm")
                    : t("sidebar.update.installArm")}
                </Button>
              </AlertAction>
            ) : null}
          </Alert>
