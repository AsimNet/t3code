...
export function AddProviderInstanceWizardSteps({
  ...
  const stepLabels = [
    t("settings.providers.wizard.step.driver"),
    t("settings.providers.wizard.step.identity"),
    t("settings.providers.wizard.step.config"),
  ];
  ...
  return (
    ...
    >
      {ADD_PROVIDER_WIZARD_STEPS.map((step, index) => {
        const stepLabel = stepLabels[index] ?? step;
        const summary = index < currentStep ? summaries[index] : null;
        return (
          <li key={step} className="min-w-0">
            <button
              ...
              aria-label={
                summary
                  ? t("settings.providers.wizard.stepAriaWithSummary", {
                      step: stepLabel,
                      number: index + 1,
                      summary,
                    })
                  : t("settings.providers.wizard.stepAria", {
                      step: stepLabel,
                      number: index + 1,
                    })
              }
