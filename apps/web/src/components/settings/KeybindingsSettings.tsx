...
function UnknownWhenVariableWarning({
  ...
  const label =
    identifiers.length === 1
      ? t("settings.keybindings.when.unknown.one", { conditions: identifiers[0] ?? "" })
      : t("settings.keybindings.when.unknown.other", {
          conditions: identifiers.join(t("settings.keybindings.listSeparator")),
        });
  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        {t("settings.keybindings.when.unknown.tooltip")}
      </TooltipPopup>
    </Tooltip>
  );
}
...
function KeybindingConflictWarning({ labels }: { labels: ReadonlyArray<string> }) {
  ...
  const commands = labels.slice(0, 3).join(t("settings.keybindings.listSeparator"));
  const description =
    labels.length === 1
      ? t("settings.keybindings.conflict.one", { commands })
      : labels.length > 3
        ? t("settings.keybindings.conflict.truncated", { commands })
        : t("settings.keybindings.conflict.other", { commands });
  ...
  return (
    <Tooltip>
      ...
      />
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        {description} {t("settings.keybindings.conflict.hint")}
      </TooltipPopup>
    </Tooltip>
  );
}
...
function WhenVariableSelect({
  ...
  return (
    <Select value={value} onValueChange={(nextValue) => nextValue && onChange(nextValue)}>
      ...
      >
        <SelectValue
          placeholder={t("settings.keybindings.when.conditionPlaceholder")}
          className="leading-7"
        />
    ...
    return (
      <div className="flex items-center gap-2 rounded-md border border-border/70 bg-background/60 px-2 py-2">
        <Toggle
          ...
          aria-label={t("settings.keybindings.when.negateCondition", {
        ...
        {onRemove ? (
          <Button
            ...
            aria-label={t("settings.keybindings.when.removeCondition")}
            onClick={onRemove}
          >
      ...
      >
        <div className="flex items-center gap-2">
          <Toggle
            ...
            aria-label={t("settings.keybindings.when.negateGroup")}
            variant="outline"
            size="xs"
            className="h-7 min-w-10 px-2 text-2xs sm:h-7"
          >
          ...
          {onRemove ? (
            <Button
              ...
              aria-label={t("settings.keybindings.when.removeNegatedGroup")}
              onClick={onRemove}
            >
              <MinusIcon className="size-3.5" />
            </Button>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {onRemove ? (
          <Button
            ...
            size="icon-sm"
            className="ms-auto size-7 sm:size-7"
            aria-label={t("settings.keybindings.when.removeGroup")}
            onClick={onRemove}
          >
            <MinusIcon className="size-3.5" />
          </Button>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-medium text-foreground">
            {t("settings.keybindings.when.title")}
          </div>
        </div>
      ...
      <div className="space-y-1.5">
        <div className="relative">
          <Input
            ...
            placeholder={t("settings.keybindings.when.always")}
            aria-invalid={Boolean(parseError)}
            aria-label={t("settings.keybindings.when.expressionAriaLabel")}
            className={cn(
      ...
      <div className="relative">
        ...
        {parseError ? (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-lg border border-destructive/30 bg-background/75 p-4 text-center text-xs text-destructive backdrop-blur-[1px]">
            {t("settings.keybindings.when.fixExpression")}
          </div>
        ) : null}
      </div>
    </div>
      ...
      <div className="flex min-w-0 items-center gap-2 pe-4">
        {showPill ? (
          <button
            ...
            aria-label={t("settings.keybindings.editShortcut", {
          ...
          <Input
            ...
            aria-label={t("settings.keybindings.keybindingFor", {
              command: commandLabel(row.command),
            })}
        ...
        {isDirty ? (
          <Button
            ...
            onClick={save}
          >
            {isSaving ? t("settings.keybindings.saving") : "Save"}
          </Button>
        ) : null}
      </div>
      <div className="pe-4">
        <Popover>
          <PopoverTrigger
            ...
            aria-label={t("settings.keybindings.editWhenClause", {
              command: commandLabel(row.command),
            })}
          >
            <span className="truncate">
              {whenDraftExpression || t("settings.keybindings.when.always")}
            </span>
            ...
            <MenuTrigger
              render={
                <Button
                  ...
                  disabled={isSaving}
                  aria-label={t("settings.keybindings.rowActions", {
                    command: commandLabel(row.command),
                  })}
                />
              }
...
function NewKeybindingTableRow({
  ...
  const commandLabelText = commandDraft
    ...
    : t("settings.keybindings.newBinding");
    ...
    <div className="grid grid-cols-[minmax(190px,1.1fr)_minmax(220px,0.85fr)_minmax(210px,1fr)_60px] items-center px-4 py-1.5 text-sm even:bg-muted/15 hover:bg-accent/40">
      <div className="min-w-0 pe-4">
        ...
        >
          ...
          >
            <SelectValue placeholder={t("settings.keybindings.commandPlaceholder")} />
          </SelectTrigger>
          <SelectContent
            alignItemWithTrigger={false}
            matchTriggerWidth={false}
      ...
      <div className="flex min-w-0 items-center gap-2 pe-4">
        <Input
          data-keybinding-capture=""
          aria-label={t("settings.keybindings.keybindingFor", { command: commandLabelText })}
          value={isRecording ? "" : keyDraft}
          placeholder={
            isRecording
              ? t("settings.keybindings.pressShortcut")
      ...
      <div className="pe-4">
        <Popover>
          <PopoverTrigger
            ...
            aria-label={t("settings.keybindings.editWhenClause", { command: commandLabelText })}
          >
            <span className="truncate">
              {whenDraftExpression || t("settings.keybindings.when.always")}
            </span>
      ...
      <div className="flex items-center justify-end gap-1">
        ...
        <Tooltip>
          ...
          <TooltipPopup side="top">{t("settings.keybindings.cancelNew")}</TooltipPopup>
        </Tooltip>
      </div>
    </div>
  );
...
export function KeybindingsSettingsPanel() {
  ...
  const openKeybindingsFile = useCallback(() => {
    ...
    void (async () => {
      ...
      toastManager.add({
        title: t("settings.keybindings.openFileFailed.title"),
        description:
          error instanceof Error
            ? error.message
            : t("settings.keybindings.openFileFailed.description"),
        type: "error",
      });
    })();
    ...
    (input: ServerUpsertKeybindingInput) => {
      ...
      void (async () => {
        ...
        if (!isAtomCommandInterrupted(result)) {
          const error = squashAtomCommandFailure(result);
          toastManager.add({
            title: t("settings.keybindings.saveFailed.title"),
            description:
              error instanceof Error
                ? error.message
                : t("settings.keybindings.saveFailed.description"),
            type: "error",
          });
        }
      })();
    ...
    (row: KeybindingRow) => {
      ...
      void (async () => {
        ...
        if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
          const error = squashAtomCommandFailure(result);
          toastManager.add({
            title: t("settings.keybindings.removeFailed.title"),
            description:
              error instanceof Error
                ? error.message
                : t("settings.keybindings.removeFailed.description"),
            type: "error",
          });
        }
      })();
  ...
  return (
    <SettingsPageContainer className="max-w-5xl">
      <SettingsSection
        title={t("settings.keybindings.section")}
        headerAction={
      ...
      >
        {!isElectron ? (
          <div className="flex items-start gap-2 border-b border-warning/20 bg-warning/5 px-3 py-2.5 text-[12px] leading-relaxed text-muted-foreground sm:px-4">
            ...
            <p>{t("settings.keybindings.browserWarning")}</p>
          </div>
        ) : null}
        ...
        >
          <div className="grid min-w-[680px] grid-cols-[minmax(190px,1.1fr)_minmax(220px,0.85fr)_minmax(210px,1fr)_60px] border-b border-border/70 bg-muted/25 px-4 py-2 text-2xs font-semibold uppercase tracking-[0.07em] text-muted-foreground">
            <div>{t("settings.keybindings.column.command")}</div>
            <div>{t("settings.keybindings.column.keybinding")}</div>
            <div>{t("settings.keybindings.column.when")}</div>
            <div>{t("settings.keybindings.column.status")}</div>
          </div>
