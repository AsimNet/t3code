    ...
    <div className="grid gap-2">
      <div className="flex items-center justify-between gap-3">
        <span className="text-xs font-medium text-foreground">
          {t("settings.providers.env.title")}
        </span>
        <Button
          type="button"
          size="sm"
        ...
        >
          ...
          {t("settings.providers.env.add")}
        </Button>
      </div>
      {rows.length === 0 ? (
        <p className="text-xs text-muted-foreground">{t("settings.providers.env.empty")}</p>
      ) : (
        <div className="overflow-hidden rounded-md border border-border/70">
          <Table>
            <TableHeader className="bg-muted/25 text-2xs text-muted-foreground">
              <TableRow className="hover:bg-transparent">
                <TableHead>{t("settings.providers.env.columnVariable")}</TableHead>
                <TableHead>{t("settings.providers.env.columnValue")}</TableHead>
                <TableHead className="w-20">
                  {t("settings.providers.env.columnSensitive")}
                </TableHead>
                <TableHead className="w-12 text-end">
                  <span className="sr-only">{t("settings.providers.env.columnOptions")}</span>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                >
                  <TableCell>
                    <DraftInput
                      ...
                      spellCheck={false}
                      aria-label={t("settings.providers.env.nameAria", { index: index + 1 })}
                    />
                  </TableCell>
                  <TableCell>
                    <DraftInput
                      ...
                      placeholder={
                        variable.valueRedacted
                          ? t("settings.providers.env.storedSecret")
                          : t("settings.providers.env.columnValue")
                      }
                      spellCheck={false}
                      aria-label={t("settings.providers.env.valueAria", { index: index + 1 })}
                    />
                  </TableCell>
                  <TableCell className="w-20">
                    <div className="flex h-8 items-center justify-center">
                      <Checkbox
                        ...
                        aria-label={t("settings.providers.env.markSensitive", {
                  ...
                  <TableCell className="w-12">
                    <div className="flex justify-end">
                      <Button
                        ...
                        aria-label={t("settings.providers.env.remove", {
                          name: variable.name || index + 1,
                        })}
                      >
      ...
      <span className="text-xs text-muted-foreground">{t("settings.providers.env.note")}</span>
    </div>
  );
}
...
export function ProviderInstanceCard({
  ...
  const { copyToClipboard } = useCopyToClipboard<{ providerName: string }>({
    onCopy: ({ providerName }) => {
      toastManager.add({
        type: "success",
        title: t("settings.providers.copy.successTitle", { provider: providerName }),
        description: t("settings.providers.copy.successDescription"),
      });
    },
    onError: (error, { providerName }) => {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.providers.copy.errorTitle", { provider: providerName }),
  ...
  const titleIconNode = driverKind ? (
    <ProviderInstanceIcon
      ...
      badgeClassName="end-[-0.125rem] bottom-[-0.125rem] h-3 min-w-3 px-0.5 text-[7px]"
    />
            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  onClick={onDelete}
                  aria-label={t("settings.providers.card.deleteAria", { instanceId })}
                >
                  <Trash2Icon className="size-3" />
                </Button>
              }
            />
            <TooltipPopup side="top">{t("settings.providers.card.deleteTooltip")}</TooltipPopup>
          </Tooltip>
        </span>
      ) : null}
    </>
  ...
  const authRowNode = (
    <p className="flex min-w-0 flex-wrap items-center gap-x-1 text-[13px] leading-[1.45] text-muted-foreground/80">
      {hasAuthenticatedEmail ? (
        <>
          <span>{t("settings.providers.auth.authenticatedAs")}</span>
        ...
        <>
          ...
          <ProviderAuthEmail
            email={authEmail}
            separator
            prefix={t("settings.providers.auth.emailPrefix")}
          />
        </>
      )}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <PopoverTrigger
                    render={
                      <Button
                        ...
                        aria-label={t("settings.providers.card.updateAvailableAria")}
                      >
                    ...
                    <div className="grid min-w-0 gap-3">
                      <div className="grid gap-0.5">
                        <p className="text-[13px] font-semibold leading-tight text-foreground">
                          {t("settings.providers.card.updateAvailable")}
                        </p>
                        <p
                          className={cn(
                        ...
                        <div className="flex min-w-0 items-center gap-1 rounded-md border border-border/70 bg-muted/40 py-0.5 pe-0.5 ps-2">
                          ...
                          <Tooltip>
                            <TooltipTrigger
                              render={
                                <Button
                                  ...
                                  aria-label={t("settings.providers.card.copyCommandAria")}
                                >
                            ...
                            <TooltipPopup side="top">
                              {t("settings.providers.card.copyCommandTooltip")}
                            </TooltipPopup>
                          </Tooltip>
                        </div>
                      ) : null}
          ...
          <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end">
            <Button
              ...
              aria-label={t("settings.providers.card.toggleDetails", { name: displayName })}
            >
              <ChevronDownIcon
            ...
            <Switch
              ...
              aria-label={t("settings.providers.card.enableAria", { name: displayName })}
            />
          </div>
        </div>
      </div>
          ...
          <div className="space-y-5 px-3 pb-4 pt-2 sm:px-4">
            <div>
              <label htmlFor={`provider-instance-${instanceId}-display-name`} className="block">
                <span className="text-xs font-medium text-foreground">
                  {t("settings.providers.card.displayName")}
                </span>
                <DraftInput
                  ...
                  placeholder={
                    driverOption?.label ?? t("settings.providers.card.displayNamePlaceholder")
                  }
                  spellCheck={false}
                />
                <span className="mt-1 block text-xs text-muted-foreground">
                  {t("settings.providers.card.displayNameHint")}
                </span>
              </label>
            </div>
            ...
            <div>
              <ProviderAccentColorPicker
                ...
                commitDelayMs={120}
                description={t("settings.providers.card.accentDescription")}
              />
            </div>
            ...
            {driverOption !== undefined ? (
              ...
              <div>
                <p className="text-xs text-muted-foreground">
                  {t("settings.providers.card.unknownDriver.prefix")}
                  <code className="text-foreground">{String(instance.driver)}</code>
                  {t("settings.providers.card.unknownDriver.suffix")}
                </p>
              </div>
            )}
          </div>
