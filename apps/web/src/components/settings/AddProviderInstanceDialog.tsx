...
function validateInstanceId(
  ...
  messages: {
    ...
    readonly duplicate: (instanceId: string) => string;
  },
): string | null {
  if (id.length === 0) return messages.required;
  if (id.length > 64) return messages.tooLong;
  if (!INSTANCE_ID_PATTERN.test(id)) {
    return messages.pattern;
  }
  if (existing.has(id)) return messages.duplicate(id);
  return null;
}
...
export function AddProviderInstanceDialog({ open, onOpenChange }: AddProviderInstanceDialogProps) {
  ...
  const instanceIdError = validateInstanceId(instanceId, existingIds, {
    required: t("settings.providers.instanceId.required"),
    tooLong: t("settings.providers.instanceId.tooLong"),
    pattern: t("settings.providers.instanceId.pattern"),
    duplicate: (instanceId) => t("settings.providers.instanceId.duplicate", { instanceId }),
  });
  const showInstanceIdError = hasAttemptedSubmit && instanceIdError !== null;
  const previewLabel =
    label.trim() || t("settings.providers.add.previewLabel", { provider: driverOption.label });
  ...
  const handleSave = () => {
    ...
    try {
      ...
      toastManager.add({
        type: "success",
        title: t("settings.providers.add.successTitle"),
        description: t("settings.providers.add.successDescription", {
      ...
      toastManager.add({
        type: "error",
        title: t("settings.providers.add.errorTitle"),
        description:
          error instanceof Error ? error.message : t("settings.providers.add.errorFallback"),
      });
    }
  };
      ...
      <DialogPopup className="max-w-xl overflow-hidden">
        <div className="flex min-h-0 flex-col overflow-hidden">
          <DialogHeader>
            ...
            <DialogDescription>{t("settings.providers.add.description")}</DialogDescription>
            ...
            <AnimatedHeight>
              <div className={cn("grid gap-2", wizardStep !== 0 && "hidden")}>
                <div id="add-instance-driver-label" className="text-sm font-medium text-foreground">
                  {t("settings.providers.add.driverLabel")}
                </div>
                <RadioGroup
                  value={driver}
                  ...
                  {COMING_SOON_DRIVER_OPTIONS.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge variant="warning" size="sm">
                          {t("settings.providers.add.comingSoon")}
                        </Badge>
                      </RadioPrimitive.Root>
                    );
                  })}
              ...
              <label className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
                <span className="text-xs font-medium text-foreground">
                  {t("settings.providers.add.labelField")}
                </span>
                <Input
                  className="bg-background"
                  placeholder={t("settings.providers.add.labelPlaceholder")}
                  value={label}
                  onChange={(event) => setLabel(event.target.value)}
                />
                <span className="text-2xs text-muted-foreground">
                  {t("settings.providers.add.labelHint")}
                </span>
              </label>

              <label className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
                <span className="text-xs font-medium text-foreground">
                  {t("settings.providers.add.instanceIdField")}
                </span>
                <Input
                  className="bg-background"
                ...
                {showInstanceIdError ? (
                  ...
                  <span className="text-2xs text-muted-foreground">
                    {t("settings.providers.add.instanceIdHint")}
                  </span>
                )}
              </label>
              ...
              <div className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
                ...
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  <input
                    ...
                    aria-label={t("settings.providers.add.accentColorAria")}
                    ...
                    {PROVIDER_ACCENT_SWATCHES.map((swatch) => {
                      ...
                      return (
                        <button
                          ...
                          aria-label={t("settings.providers.add.useSwatch", { color: swatch })}
                        />
                      );
                    })}
                  </div>
                  {accentColor ? (
                    ...
                    >
                      {t("settings.providers.add.clear")}
                    </Button>
                  ) : null}
                </div>
                <span className="text-2xs text-muted-foreground">
                  {t("settings.providers.add.accentHint")}
                </span>
              </div>
              ...
              {driverSettingsFields.length > 0 ? (
                ...
                <div className="grid gap-2">
                  <p className="text-sm text-muted-foreground">
                    {t("settings.providers.add.noConfig")}
                  </p>
                </div>
              ) : null}
            </AnimatedHeight>
